...
import CreationGoal, { ICreationGoal } from '../models/CreationGoal';
import { IUser } from '../models/userModel';

/*** OTHER types of GET METHODS ********************************************************************************************************************/
// #1 extra "Get" method to search Public creationGoals by a term within their goal section
export const getPublicSearchResults = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    // #swagger.ignore = true
  try{
    const searchQuery: string = await req.query.query as string; // Capture the search term
...
export const getUserCreationGoals = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  try {
    ...
    const user: IUser | undefined  = creationGoals[0]?.user as IUser;
...
export const getUsersCreationGoalById = async (req: Request, res: Response): Promise<void> => { 
    /* #swagger.security = [{ "bearerAuth": [] }] */
  ...
  try {
    ...
    if (!req.user || creationGoal.user.toString() !== req.user.id) {
...
export const getAddForm = (req: Request, res: Response): void => { 
    /* #swagger.security = [{ "bearerAuth": [] }] */
...
export const getCreationGoalById= async (req: Request, res: Response): Promise<void> => {
    // #swagger.ignore = true
  try {
...
export const addCreationGoal = async (req: Request, res: Response): Promise<void> => {
    /* #swagger.security = [{ "bearerAuth": [] }] */
  ...
  try {
      ...
      */   
    req.body.user = (req.user as IUser).id    
...
export const updateCreationGoal = async (req: Request, res: Response): Promise<void> => {
  /* #swagger.security = [{ "bearerAuth": [] }] */  
  ...
  */
    ...
    let creationGoal = await CreationGoal.findById(req.params.id).lean();
    const creationGoals = await CreationGoal.find({ user: userId }).populate('user').lean();

    if (!creationGoal) {
      return res.status(404).render('error/404')
    }

    if (creationGoal.user.toString() !== userId) {
      ...
      res.status(200).render('dashboard', {      
        name: `${(req.user as IUser).firstName} ${(req.user as IUser).lastName}`,      
        creationGoals,     
...
export const deleteCreationGoal = async (req: Request, res: Response): Promise<void> => {
    /* #swagger.security = [{ "bearerAuth": [] }] */
  ...
  try {
    const userId = (req.user as IUser).id;
    let creationGoal = await CreationGoal.findById(req.params.id).lean() as ICreationGoal | null;
    ...
    if (creationGoal.user.toString() !== req.user.id) {
