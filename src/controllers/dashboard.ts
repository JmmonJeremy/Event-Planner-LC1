...
export const getAllUsersCreationGoals = async (req: Request, res: Response): Promise<void> => {
...
try {  
  const creationGoals: ICreationGoal[] = await CreationGoal.find({ user: req.user.id }).populate('user').lean<ICreationGoal[]>();  
  // Check if the `registered` query parameter is true  
  const registered: boolean = req.query.registered === 'true';  
  // Check if the `created` query parameter is true
  const created: boolean = req.query.created === 'true';
  // Check if the `created` query parameter is true
  const updated: boolean = req.query.updated === 'true';
  // Check if the `created` query parameter is true
